...
{
    public class StoreProcedureDataAccessInjector<k, h>
        ...
        where h : Entity
    {
        ...
        /// <returns></returns>
        public static ICrudDataAccess<h> GetDatabaseInstance(string Name, string Container, string Section, string StoredProcedure)
        {
            ...
            if (_instance == null)
            {
                lock (synlock)
                {
                    ...
                    container.RegisterType<k>(Name, new InjectionMember[] { new InjectionConstructor(new ResolvedParameter(typeof(Database), Name), StoredProcedure) });
